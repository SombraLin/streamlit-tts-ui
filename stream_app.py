...
# 文本转语音
async def VolceTextToSpeech(text, voices, speed, volume, pitch):
    output_file = "output.mp3"
    voices = VOICER_LIST[voices]
    audio_file_paths = await volce_tts_stream.volce_tts(text, voices, "happy", speed, volume, pitch, output_file)
    return audio_file_paths

# 同步执行异步文本转语音函数
def sync_VolceTextToSpeech(text, voices, rate, volume, pitch):
    return asyncio.run(VolceTextToSpeech(text, voices, rate, volume, pitch))

# Streamlit App
st.title("微软 Edge 文本转语音")
...
if st.button("生成音频"):
    # 调用文本转语音函数并生成音频
    generated_files = sync_VolceTextToSpeech(text, voices, rate, volume, pitch)
